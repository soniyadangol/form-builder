...
export const newForm = (data => {
    ...
    data.forEach(input => {
        switch(input.type) {
            case 'textarea':
                let formField = cInputTextarea(input.label, input.rows, input.cols, input.max, input.min);
                form.innerHTML += formField;
                break;
            default:
                formField = cInput(input.label, input.type, input.min, input.max);
                form.innerHTML += formField;
                if(input.isRequired) {
                    addValidation(form.querySelectorAll('.form-control'));
                    addValidationOnKeyUp(form.querySelectorAll('.form-control'))
                }
                break;
        }
    });
