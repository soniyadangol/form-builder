...
const defaultInput = (form, input) => {
    const formField = `
        <div class="form-group">
            <label for="${input.label}" class="fb-label">${input.label}</label>
            <input type="${input.type}" placeholder="${input.placeholder}" class="form-control fb-input" name="${input.label ? input.label : 'Field'}" max="${input.max}" min="${input.min}" mandatory="${input.isRequired}" valid="${input.valid}">
            <p class="error-message"></p>
        </div>
    `;
    form.innerHTML += formField;
