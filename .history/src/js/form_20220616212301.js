import { cInput, cInputTextarea, cButton, cCheckBox, cRadioButton, cSelect, cToggleSwitch } from "./shared";
import { addValidation, addValidationOnKeyUp, checkValidity } from "./validation";
...
const inputButton = (form, input) => {
    const formField = cButton(input.value, input.type);
...
const defaultInput = (form, input) => {
    const formField = `
        <div class="form-group">
            <label for="${input.label}" class="fb-label">${input.label}</label>
            <input type="${input.type}" placeholder="${input.placeholder}" class="form-control fb-input" name="${input.label}" max="${input.max}" min="${input.min}" mandatory="${input.isRequired}">
            <p class="error-message"></p>
        </div>
    `;
    form.innerHTML += formField;
