...
export const newForm = (data => {
    ...
    container.innerHTML += fetchStepper(formActive='true');
...
function onMainFormSubmit(data) {
    ...
    for(let i=0; i< formControl.length; i++) {
        isFormValid = formControl[i].hasAttribute('valid');
    }
    ...
    if(isFormValid) {
        ...
        displayContainer.classList.add('container','container--md');
        mainWrapper.appendChild(displayContainer);

        displayContainer.innerHTML += fetchStepper(detailActive=true);
