        ...
        const inputs = inputWrapper.querySelectorAll('.field-container .form-control');
            submitButton.addEventListener('click', event => {
                ...
                inputs.forEach((input, index) => {
                    checkboxObj["checkbox" + (index + 1)] = input.value 
                    checkboxObj.type = 'checkbox'
                });
                inputArray.push(checkboxObj);
                console.log(inputArray);
                event.target.parentElement.parentElement.parentElement.remove();
                showInputValueOnDom(checkboxObj);
            });
        }
...
export { getInputText, getTextArea, getInputType, checkboxObj };