...
const showCheckboxAndRadio = type => {
    value.data.map(checkBox => {
        console.log(checkBox)
            html = `
                <input type="checkbox" name="${checkBox.name}" value="${checkBox.name}" disabled>
                <label for="${checkBox.name}"> ${checkBox.value}</label><br>
            `;
...
export const showInputValueOnDom = (value => {
    ...
    switch(value.type) {
        ...
        case 'checkbox':
            showCheckboxAndRadio(value.type);
            break;
