...
export const showInputValueOnDom = (value => {
    ...
    switch(value.type) {
        ...
        case 'checkbox':
            ...
            for(let i=0; i<value.checkBoxes.length; i++) {
                html = `
                    <input type="checkbox" id="${value.checkBoxes}${i}" name="${value.checkBoxes.name}" value="${value.checkBoxes.name}">
                    <label for="${value.checkBoxes.name}"> ${value.checkBoxes.value}</label><br>
                `;
                listWrapper.innerHTML += html;
            }
            break;
